...
score: 8.0
evidence: 复杂几何体中流体流动的PINN构建
tldr: "针对标准PINN在复杂几何结构（如TPMS）中因逐点残差最小化导致的收敛困难和物理守恒性差等问题，本文提出MUSA-PINN。该方法将PDE约束重构为分层球形控制体上的积分守恒定律，结合三尺度子域策略与两阶段训练方案，显著增强了复杂流道内的全局信息传播。实验表明，该方法在保持质量守恒的同时，将流场预测误差降低了高达93%。"
...
result: "在TPMS几何结构的稳态不可压缩流测试中，MUSA-PINN显著优于现有基准，相对误差最高降低93%并实现了优异的质量守恒。"
conclusion: 通过将积分守恒律与多尺度子域策略相结合，MUSA-PINN为解决复杂几何中的流体动力学仿真提供了高效且物理一致的新方案。
---
