...
result: 识别了六种智能体治理失效类型，并为五类机构参与者制定了具体的治理机制、责任分配及评估指标。
conclusion: 军事AI治理必须从二元控制观转向在整个运行生命周期内主动测量和管理控制质量的持续模型。
---

## 摘要
智能体 AI 系统（具备目标解释、世界建模、规划、工具使用、长时程运行及自主协作能力）引入了现有安全框架尚未解决的独特控制失效问题。我们识别了与这些能力相关的六种智能体治理失效，并展示了它们如何在军事场景中削弱实质性的人类控制。我们提出了“智能体军事 AI 治理框架”（AMAGF），这是一个围绕三大支柱构建的可衡量架构：预防性治理（降低失效可能性）、探测性治理（实时检测控制退化）以及纠正性治理（恢复或安全降级运行）。其核心机制“控制质量评分”（CQS）是一个量化人类控制的综合实时指标，能够在控制力减弱时实现分级响应。针对每种失效类型，我们定义了具体机制，在五个机构主体之间分配了职责，并使评估指标正式化。通过一个实际操作场景说明了该框架的实施过程，并将该框架置于既有的智能体安全文献体系中。我们认为，治理必须从二元控制观念转向连续模型，在整个运行生命周期中主动衡量和管理控制质量。
