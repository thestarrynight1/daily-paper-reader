...
tldr: 针对分布式风力发电机数据隐私和异构性挑战，本文提出一种行为感知的联邦学习预测框架。该框架采用两阶段策略：首先利用双轮盘赌选择（DRS）和递归自动拆分算法根据长期行为统计数据对风机进行聚类，随后通过FedAvg训练特定集群的LSTM模型。在丹麦400台风机上的实验证明，该方法在保护隐私的同时，预测精度优于地理分区并比肩强基准模型，为异构分布式风机群提供了实用的预测方案。
selection_source: fresh_fetch
motivation: 传统的集中式风电预测面临数据隐私泄露、传输成本高以及风机异构性带来的挑战。
method: 提出一种两阶段联邦学习框架，结合DRS初始化聚类与FedAvg算法训练集群专属的LSTM预测模型。
result: 在丹麦400台独立风机的实验中，该框架成功识别了行为一致的群体，预测精度优于地理分区方法。
conclusion: 行为感知的聚类联邦学习是解决异构分布式风机群短期功率预测的一种兼顾隐私与性能的有效方案。
---

## 摘要
准确的短期风电功率预测对于电网调度和市场运营至关重要，但集中处理涡轮机数据会引发隐私、成本和异构性问题。我们提出了一种两阶段联邦学习框架，首先利用双轮盘赌选择（DRS）初始化结合递归自动拆分（Auto-split）优化，根据长期行为统计数据对涡轮机进行聚类，然后通过 FedAvg 训练特定于集群的 LSTM 模型。对丹麦 400 台独立涡轮机的实验表明，DRS-auto 能够发现行为一致的群体，并在保持数据本地性的同时实现具有竞争力的预测精度。行为感知分组的表现始终优于地理分区，并能达到与强 k-means++ 基准线相当的效果，这为异构分布式涡轮机群提供了一种实用的隐私友好型解决方案。
